        ...
        <>
            <DataTable<PremiumAmenity>
                ...
                renderGridCard={(amenity) => (
                    <div key={amenity?._id} className="overflow-hidden bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-md transition-all">
                        <div className="relative h-50 bg-slate-50 flex items-center justify-center">
                            {amenity.icon ? (
                                <CustomImage
                                    src={amenity.icon}
                                    alt={amenity.name}
                                    className="h-full w-full object-contain p-4 transition-all duration-500 ease-in-out"
                                />
                            ) : (
                                <ImageIcon size={48} className="text-slate-300" />
                            )}
                            <span
                                className={`absolute left-3 top-3 px-2 py-1 rounded-full text-xs font-medium ${amenity.status === 1
