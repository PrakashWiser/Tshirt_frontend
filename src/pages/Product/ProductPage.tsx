...
const getFirstVariantImage = (product: ProductItem): string => {
...
export default function ProductPage() {
  ...
  const handleSubmit = async (values: Record<string, any>) => {
    ...
    formData.append("isBestSeller", String(Boolean(values.isBestSeller)));
    formData.append("isNewArrival", String(Boolean(values.isNewArrival)));
    formData.append("isTrending", String(Boolean(values.isTrending)));
    ...
    (values.variants || []).forEach((v: any, index: number) => {
      const variantImages = Array.isArray(v.images) ? v.images : [];
      variantImages.forEach((item: any) => {
        if (item instanceof File) {
          formData.append(`variantImages[${index}]`, item);
        }
      });
    });
  ...
  const columns: ColumnDef<ProductItem>[] = [
    {
      key: "image",
      header: "Image",
      accessor: (row) => getFirstVariantImage(row),
      width: 90,
      render: (value, row) => {
    ...
    },
    {
      ...
      render: (_, row) => (
        <div onClick={(e) => e.stopPropagation()}>
          <DotMenu
            onView={() => navigate(`/products/${row.slug || row._id}`)}
            onEdit={() => openEdit(row)}
      ...
      {!isFormOpen && (
        <DataTable
          ...
          renderGridCard={(product: ProductItem) => {
            const image = getFirstVariantImage(product);
