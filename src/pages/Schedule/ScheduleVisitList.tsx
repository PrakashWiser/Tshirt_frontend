...
import { Download, Calendar, Clock, User, Phone, Home, Video, Users } from "lucide-react";
...
const STATUS_OPTIONS: { value: ScheduleVisitStatus; label: string }[] = [
...
export default function ScheduleVisitList() {
    ...
    const { message, error, scheduleVisits, isLoading, pagination } = useAppSelector(
        (state) => state.scheduleVisit
    );
    ...
    useEffect(() => {
        dispatch(getAllScheduleVisits({
            page: currentPage,
            limit,
            status: statusFilter as ScheduleVisitStatus || undefined
        }));
    ...
    useEffect(() => {
        if (message) {
            ...
            dispatch(getAllScheduleVisits({ page: currentPage, limit }));
    ...
    const handleStatusChange = (id: string, newStatus: string) => {
        dispatch(updateScheduleVisitStatus({ id, status: newStatus as ScheduleVisitStatus }));
    };

    const columns = useMemo<ColumnDef<ScheduleVisitTableRow>[]>(
        () => [
            {
                key: "propertyName",
                header: "Property",
            ...
            },
            {
                ...
                render: (value) => {
                    ...
                    const Icon = type === "In Person" ? Users : Video;
                    return (
                        <span className={`px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 ${getViewingTypeColor(type)}`}>
                            <Icon size={12} />
                            {type}
                        </span>
                    );
    ...
    const tableData: ScheduleVisitTableRow[] = useMemo(() => {
        return scheduleVisits.map((sv) => ({
            id: sv._id,
            propertyName: typeof sv.propertyId === "object"
                ? sv.propertyId.name
                : "Unknown",
            propertyId: typeof sv.propertyId === "object"
                ? sv.propertyId._id
                : sv.propertyId || "",
    ...
    const handleExport = () => {
        ...
        exportTableData(tableData as any, exportColumns, "ScheduleVisits");
    };
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-white border border-slate-200 rounded-xl p-4">
                        ...
                        <p className="text-2xl font-bold text-slate-900">
                            {pagination?.total || scheduleVisits.length}
                        </p>
                    </div>
                    <div className="bg-white border border-slate-200 rounded-xl p-4">
                        <p className="text-sm text-slate-500">Active</p>
                        <p className="text-2xl font-bold text-green-600">
                            {scheduleVisits.filter((sv) => sv.status === "Active").length}
                        </p>
                    </div>
                    <div className="bg-white border border-slate-200 rounded-xl p-4">
                        <p className="text-sm text-slate-500">Completed</p>
                        <p className="text-2xl font-bold text-blue-600">
                            {scheduleVisits.filter((sv) => sv.status === "Completed").length}
                        </p>
                    </div>
                </div>
                    ...
                    <div className="p-4">
                        {isLoading ? (
                            ...
                            <DataTable
                                ...
                                rowKey="id"
                                defaultView="table"
                                searchKeys={["propertyName", "fullName", "email", "phone"]}
